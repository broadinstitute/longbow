...
def test_sift_from_file(tmpdir, input_bam, expected_bam):
    actual_bam = tmpdir.join("sift_actual_out.bam")
    args = ["sift", "-f", "-o", actual_bam, str(input_bam)]
...
@pytest.mark.parametrize("input_bam, expected_bam", TEST_PARAMS)
def test_sift_from_pipe(tmpdir, input_bam, expected_bam):
    actual_bam = tmpdir.join("sift_actual_out.pipe.bam")
    args = ["sift", "-f", "-o", actual_bam]
