...
def test_extract_from_file(tmpdir, input_bam):
    actual_file = tmpdir.join("sift_actual_out.bam")
    args = ["sift", "-f", "-o", actual_file, str(input_bam)]
...
def test_extract_from_pipe(tmpdir, input_bam):
    actual_file = tmpdir.join("sift_actual_out.pipe.bam")

    args = ["sift", "-f", "-o", actual_file]
