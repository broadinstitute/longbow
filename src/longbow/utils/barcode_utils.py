...
def _load_barcode_allowlist_helper(open_file_handle, barcode_set, disable_pbar=None):
    ...
    for line in tqdm(
        open_file_handle,
        desc="Loading barcode allow list",
        unit=" barcode",
        colour="green",
        file=sys.stderr,
        leave=True,
...
def generate_symspell_index(
    wl_fname, max_dist_allowed, prefix_len=16, freq_list_sep="\t"
):
    # Code generously donated from Victoria Popic
    sym_spell = SymSpell(
        max_dictionary_edit_distance=max_dist_allowed, prefix_length=prefix_len
    )
    sym_spell._distance_algorithm = (
        symspellpy.editdistance.DistanceAlgorithm.LEVENSHTEIN_FAST
    )
    sym_spell.load_dictionary(
        wl_fname, 0, 1, separator=freq_list_sep
    )  # positions of the term and count, respectively
    return sym_spell
...
def find_match_symspell(barcode, barcode_allow_list, sym_spell_index, dist_thr):
    ...
    matches = sym_spell_index.lookup(
        barcode, Verbosity.CLOSEST, max_edit_distance=dist_thr
    )
    if not matches:
