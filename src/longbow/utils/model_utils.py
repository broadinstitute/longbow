...
class ModelBuilder:
    ...
    def make_global_alignment_model(target, name=None):
        ...
        for c in range(len(target)):
            ...
            mc = State(
                DiscreteDistribution(
                    {
                        "A": ModelBuilder.PER_BASE_MATCH_PROB
                        if target[c] == "A"
                        else ModelBuilder.PER_BASE_MISMATCH_PROB,
                        "C": ModelBuilder.PER_BASE_MATCH_PROB
                        if target[c] == "C"
                        else ModelBuilder.PER_BASE_MISMATCH_PROB,
                        "G": ModelBuilder.PER_BASE_MATCH_PROB
                        if target[c] == "G"
                        else ModelBuilder.PER_BASE_MISMATCH_PROB,
                        "T": ModelBuilder.PER_BASE_MATCH_PROB
                        if target[c] == "T"
                        else ModelBuilder.PER_BASE_MISMATCH_PROB,
                    }
                ),
        ...
        model.add_transition(
            model.start, s[f"{name}:I0"], ModelBuilder.MATCH_INDEL_PROB
        )
        model.add_transition(
            model.start, s[f"{name}:D1"], ModelBuilder.MATCH_INDEL_PROB
        )
        model.add_transition(
            model.start, s[f"{name}:M1"], ModelBuilder.MATCH_MATCH_PROB
        )

        model.add_transition(
            s[f"{name}:I0"], s[f"{name}:I0"], ModelBuilder.INDEL_CONTINUATION_PROB
        )
        model.add_transition(
            s[f"{name}:I0"], s[f"{name}:D1"], ModelBuilder.INDEL_SWITCH_PROB / 2
        )
        model.add_transition(
            s[f"{name}:I0"], s[f"{name}:M1"], ModelBuilder.INDEL_SWITCH_PROB / 2
        )
        ...
        for c in range(1, len(target)):
            ...
            model.add_transition(
                s[f"{name}:D{c}"],
                s[f"{name}:I{c}"],
                ModelBuilder.INDEL_CONTINUATION_PROB,
            )
            model.add_transition(
                s[f"{name}:D{c}"],
                s[f"{name}:M{c + 1}"],
            ...
            model.add_transition(
                s[f"{name}:I{c}"], s[f"{name}:I{c}"], ModelBuilder.INDEL_SWITCH_PROB / 2
            )
            model.add_transition(
                s[f"{name}:I{c}"],
                s[f"{name}:M{c + 1}"],
            ...
            model.add_transition(
                s[f"{name}:M{c}"], s[f"{name}:D{c + 1}"], ModelBuilder.MATCH_INDEL_PROB
            )
            model.add_transition(
                s[f"{name}:M{c}"], s[f"{name}:I{c}"], ModelBuilder.MATCH_INDEL_PROB
            )
            model.add_transition(
                s[f"{name}:M{c}"], s[f"{name}:M{c + 1}"], ModelBuilder.MATCH_MATCH_PROB
            )

        model.add_transition(
            s[f"{name}:D{len(target)}"],
            s[f"{name}:I{len(target)}"],
            ModelBuilder.INDEL_CONTINUATION_PROB,
        )
        model.add_transition(
            s[f"{name}:D{len(target)}"], model.end, ModelBuilder.INDEL_SWITCH_PROB
        )
        ...
        model.add_transition(
            s[f"{name}:M{len(target)}"],
            s[f"{name}:I{len(target)}"],
            ModelBuilder.MATCH_TRAIL_INSERT_PROB,
        )
        model.add_transition(
            s[f"{name}:M{len(target)}"], model.end, ModelBuilder.MATCH_END_PROB
        )
    ...
    def make_homopolymer_repeat_model(name, nucleotide, expected_length):
        logger.debug(
            "Making Model: HOMOPOLYMER_REPEAT (%s:%s x %d)",
            name,
            nucleotide,
            expected_length,
        )
        ...
        bookend_state_distribution = DiscreteDistribution(
            {
                "A": ModelBuilder.HPR_BOOKEND_MATCH_PROB
                if nucleotide == "A"
                else ModelBuilder.HPR_BOOKEND_MISMATCH_PROB,
                "C": ModelBuilder.HPR_BOOKEND_MATCH_PROB
                if nucleotide == "C"
                else ModelBuilder.HPR_BOOKEND_MISMATCH_PROB,
                "G": ModelBuilder.HPR_BOOKEND_MATCH_PROB
                if nucleotide == "G"
                else ModelBuilder.HPR_BOOKEND_MISMATCH_PROB,
                "T": ModelBuilder.HPR_BOOKEND_MATCH_PROB
                if nucleotide == "T"
                else ModelBuilder.HPR_BOOKEND_MISMATCH_PROB,
            }
        )
        ...
        model.add_transition(
            s[f"{name}:M1"], model.end, ModelBuilder.HPR_SUDDEN_END_PROB
        )
        model.add_transition(
            s[f"{name}:M1"], model.start, ModelBuilder.HPR_MODEL_RECURRENCE_PROB
        )
        ...
        for c in range(1, expected_length):
            ...
            model.add_transition(
                s[f"{name}:I{c}"], model.start, ModelBuilder.HPR_MODEL_RECURRENCE_PROB
            )
            model.add_transition(
                s[f"{name}:I{c}"], model.end, ModelBuilder.HPR_SUDDEN_END_PROB
            )
        ...
        model.add_transition(
            s[f"{name}:M{expected_length}"],
            s[f"{name}:I{expected_length}"],
            ModelBuilder.MATCH_TRAIL_INSERT_PROB,
        )
        model.add_transition(
            s[f"{name}:M{expected_length}"], model.end, ModelBuilder.MATCH_END_PROB
        )
        model.add_transition(
            s[f"{name}:M{expected_length}"], model.start, ModelBuilder.MATCH_END_PROB
        )
    ...
    def make_fixed_length_random_segment(name, length):
        ...
        for i in range(1, length):
            model.add_transition(
                state_dict[f"{name}:M{i}"], state_dict[f"{name}:M{i + 1}"], 1
            )
    ...
    def connect_terminals(
        base_hmm, adapter_name_i, adapter_name_j, transition_probability=1.0
    ):
