...
def main(
    ...
    max_ccs_gc_diff,
    max_clr_gc_diff,
    max_ccs_umi_length_delta,
    max_clr_umi_length_delta,
    ...
    cached_file_names = [
        f"{input_bam.name}.locus2reads.pickle",
        os.path.basename(f"{input_bam.name}.locus2reads.pickle"),
    ]

    if (
        cache_read_loci
        and input_bam.seekable()
        and any([os.path.exists(p) for p in cached_file_names])
    ):
        ...
        locus2reads = extract_read_groups(
            input_bam,
            umi_length,
            pre_extracted,
            max_ccs_umi_length_delta,
        ...
        if cache_read_loci:
            ...
            resolved_cache_name = os.path.basename(
                f"{input_bam.name}.locus2reads.pickle"
            )
    ...
    for locus in tqdm(
        locus2reads,
        desc="Processing each locus",
        unit=" locus",
        colour="green",
        position=1,
        file=sys.stderr,
        total=len(locus2reads),
        leave=False,
        disable=not sys.stdin.isatty(),
    ):
        process_reads_at_locus(
            locus2reads[locus],
            read2umi,
            umi_length,
            max_ccs_edit_dist,
    ...
    with pysam.AlignmentFile(
        ...
        out_header = pysam.AlignmentHeader.from_dict(
            bam_utils.create_bam_header_with_program_group(
                PROG_NAME,
                input_bam_file.header,
            )
        )

        with pysam.AlignmentFile(
            output_bam, "wb", header=out_header
        ) as correct_umi_bam, pysam.AlignmentFile(
    ...
    t_end = time.time()
    logger.info(
        f"Done. Elapsed time: {t_end - t_start:2.2f}s. "
...
def valid_gene(read, gene_tag):
    ...
    return (MAS_GENE_PREFIX in read.get_tag(gene_tag)) or (
        GENCODE_GENE_PREFIX in read.get_tag(gene_tag)
    )
...
def read_passes_filters(
    ...
    max_umi_delta_filter = (
        max_final_ccs_umi_length_delta
        if ReadType(get_read_type(read)) == ReadType.CCS
        else max_final_clr_umi_length_delta
    )
    return (
        get_back_aln_score(read, back_alignment_score_tag) >= min_back_seg_score
        and abs(len(read.get_tag(final_umi_tag)) - umi_length) <= max_umi_delta_filter
    )


def extract_read_groups(
    input_bam_fname,
    umi_length,
    pre_extracted,
    ccs_max_umi_len_delta,
    clr_max_umi_len_delta,
    umi_tag,
    gene_tag,
    eq_class_tag,
    ...
    n_total_reads = 0
    with pysam.AlignmentFile(
        input_bam_fname, "rb", check_sq=False, require_index=False
    ) as input_bam:
        for read in tqdm(
            input_bam,
            desc="Extracting Read Groups",
            unit=" read",
            total=total_num_reads,
            position=0,
        ):
            ...
            if not valid_gene(read, gene_tag):
                ...
                continue
            if not valid_umi(
                read, umi_length, ccs_max_umi_len_delta, clr_max_umi_len_delta, umi_tag
            ):
                n_filtered_umi += 1
                continue
            n_valid_reads += 1
...
def get_conversion_type(source, target):
    return (
        ReadType.CCS
        if ((source.type == target.type) and (source.type == ReadType.CCS))
        else ReadType.CLR
    )
...
def can_convert(
    ...
    max_edit_dist = (
        max_ccs_edit_dist if conversion_type == ReadType.CCS else max_clr_edit_dist
    )
    ...
    max_len_diff = (
        max_ccs_length_diff if conversion_type == ReadType.CCS else max_clr_length_diff
    )
    max_gc_diff = (
        max_ccs_gc_diff if conversion_type == ReadType.CCS else max_clr_gc_diff
    )
...
def min_vertex_cover(read_ids, graph, target2umi_counts, target2umi_seq):
    ...
    while len(read_ids) > 0:
        ...
        max_target_id = max(
            ...
            key=lambda t: (
                len(graph[t]),
                max(target2umi_counts[t], key=target2umi_counts[t].get)
                == target2umi_seq[t],
            ),
        )
...
def process_reads_at_locus(
    ...
    graph, target2umi_counts, target2umi_seq = build_graph(
        reads,
        max_ccs_edit_dist,
        max_clr_edit_dist,
        max_ccs_length_diff,
