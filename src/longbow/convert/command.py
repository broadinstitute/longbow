...
def main(output_bam, default_rq, read_group_id, sample_name, min_length, input_spec):
    ...
    h = bam_utils.create_bam_header_with_program_group(logger.name,
                                                       pysam.AlignmentHeader(),
                                                       rg_dict={'ID': read_group_id, 'SM': sample_name})
    ...
    with pysam.AlignmentFile(output_bam, "wb", header=h) as bam_out:
        for file in files:
            with pysam.FastxFile(file) as fh:
                for entry in fh:
                    num_reads_seen += 1
                    #d = dict(x.split("=") for x in entry.comment.split(" "))
